...
anisanta = parseFloat(
  window.getComputedStyle(santa, null).getPropertyValue("animation-duration")
);
anigift = parseFloat(
  window.getComputedStyle(divabove, null).getPropertyValue("animation-duration")
);
...
setInterval(() => {
  ...
  if (scoreval >= 1 && scoreval < 5 && santa.style.animationDuration < "4s") {
    ...
    scoreval >= 20 &&
    scoreval < 25 &&
    santa.style.animationDuration < "2s"
  ) {
    setTimeout(() => {
